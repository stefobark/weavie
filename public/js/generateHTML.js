...
function everything(numWarps, numWefts, numTreadles) {
    ...
    */
    ...
    for (i = 0; i < numWefts; i++) {
        ...
        if (i > 0) {
            var weaveTop = weaveTop + 15;
        }

        $("#weaveBox").append('<div class="weft' + i + ' base" style="position:absolute; width:' + boxWidth + 'px; height: 15px; top:' + weaveTop + 'px; right:0;"></div>');
        $(".weftColorGrid").append('<button id="' + i + '" class="weftColorButton" style="position:absolute;  top:' + weaveTop + 'px; width:15px; height:15px;"></button>');
