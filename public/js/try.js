...
function everything(numWarps, numWefts) {		
	...
	for(i = 0; i < tieNum; i++) { 
		...
		for(c = 0; c < colNum; c++) {
			...
			$('.tie' + i).append('<button id="tRow' + c +'" class="tieButton" style="position: absolute; top:' + tieTop + '%; width:100%; height:25%;"></button>');
			}
		}
	...
	//hGrid maker 
		...
		for (i = 0; i < numWarps; i++) { 
			...
			$(".hGrid").append('<div id="' + i + '" style="position:absolute; top:0; right:' + left + 'px; height:100%; width:15px;" ><button class="hGridRow0"></button><button class="hGridRow1"></button><button class="hGridRow2"></button><button class="hGridRow3"></button></button>');			
		}
